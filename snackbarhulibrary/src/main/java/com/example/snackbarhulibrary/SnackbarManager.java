...
/**
 * Created by huyongqiang on 2016/11/15.
 */
...
class SnackbarManager {
    ...
    public void show(int duration, Callback callback) {
        synchronized (mLock) {
            ...
            if (mCurrentSnackbar != null && cancelSnackbarLocked(mCurrentSnackbar, SnackbarHu.Callback.DISMISS_EVENT_CONSECUTIVE)) {
    ...
    private void scheduleTimeoutLocked(SnackbarRecord r) {
        if (r.duration == SnackbarHu.LENGTH_INDEFINITE) {
        ...
        } else if (r.duration == SnackbarHu.LENGTH_SHORT) {
    ...
    private void handleTimeout(SnackbarRecord record) {
        synchronized (mLock) {
            if (mCurrentSnackbar == record || mNextSnackbar == record) {
                cancelSnackbarLocked(record, SnackbarHu.Callback.DISMISS_EVENT_TIMEOUT);
            }
        }
    }
